...
async fn full_sdk_lifecycle() {
    ...
    let schema = Schema::builder()
        ...
        .build();
    client.create_index(INDEX, &schema).await.expect("create_index");
    ...
    let gen_id = client
        .create_document(INDEX, &json!({ "title": "Auto id", "body": "second doc", "views": 1 }))
        .await
        .expect("create_document");
    assert!(!gen_id.is_empty());

    // get_document
    let doc = client.get_document(INDEX, "p1").await.expect("get_document");
    assert!(doc.found);
    ...
    let res = client
        .search(INDEX, &SearchRequest::new(QueryClause::match_field("body", "hybrid retrieval")))
        .await
        .expect("match search");
    assert!(res.hits.hits.iter().any(|h| h.id == "p1"));
    assert!(res.hits.hits[0].source.is_some(), "document hit carries _source");

    // knn search → chunk hit (requires the server's embedder)
    let res = client
        .search(INDEX, &SearchRequest::new(QueryClause::knn("embedding", "lexical and semantic fusion")))
        .await
        .expect("knn search");
    assert!(!res.hits.hits.is_empty(), "knn returns chunks");
    let top = &res.hits.hits[0];
    assert!(top.chunk_id.is_some() && top.content.is_some(), "chunk hit shape");
    ...
    let evolved = Schema::builder()
        ...
        .keyword("tags")
        .build();
    let changes = client.update_mapping(INDEX, &evolved).await.expect("update_mapping");
    ...
    // delete_document
    client.delete_document(INDEX, "p1").await.expect("delete_document");
    let missing = client.get_document(INDEX, "p1").await;
    assert!(missing.is_err() && missing.unwrap_err().is_not_found(), "deleted doc → 404");

    // delete_index, then it's gone
    client.delete_index(INDEX).await.expect("delete_index");
    let gone = client.get_index(INDEX).await;
    assert!(gone.is_err() && gone.unwrap_err().is_not_found(), "deleted index → 404");
}
